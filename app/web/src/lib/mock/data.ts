...
/*
  추출 실패 사유(`extract_error`). 실서버는 백엔드가 판정해 한국어 완성 문장으로 주고,
  프론트는 그대로 띄운다. 목도 같은 두 갈래를 흉내낸다.
*/

/** 사진만 있는 스캔본. 재추출해도 결과가 같다는 사실까지 문장이 말한다. */
export const MOCK_SCAN_EXTRACT_ERROR =
  '글자 정보가 없는 스캔본(사진) PDF 입니다. 문항 번호를 읽을 수 없어 0문항으로 등록했습니다. [문제 다시 추출] 을 눌러도 결과는 같습니다. 한글·워드에서 PDF 로 내보낸 파일(글자를 선택·복사할 수 있는 PDF)을 올려 주세요.';

/** 글자는 있는데 문항 번호 앵커를 못 찾은 경우(재추출이 의미 있는 쪽). */
export const MOCK_ANCHOR_EXTRACT_ERROR =
  "문제 번호 앵커를 찾지 못했습니다. '1.' '2.' 형태의 문항 번호가 있는 시험지인지 확인하세요.";
